...
int main()
{
    ...
    while (run)
    {
        ...
        {
            printf("FSW timestamp: %05.3f s\t\t|\t", (float)lora_recv_data.msg.timestamp / 1000);
            printf("LORA freq: %02d Hz\t|\t", lora_recv_data.freq);
            printf("LORA recv/send state: %01d / %01d\n", lora_recv_data.msg.fsw_state.state, lora_send_data.msg.fsw_state);
            last_print_time = now;
        }
        ...
        {
            if (lora_recv_data.msg.fsw_state.state > lora_send_data.msg.fsw_state)
            {
                pthread_mutex_lock(&lora_send_data.lock);
                lora_send_data.msg.fsw_state = lora_recv_data.msg.fsw_state.state;
        ...
        if (panel.res_4_btn && lora_send_data.msg.fsw_state != SM_STATE_INIT)
        {
        ...
        switch (lora_recv_data.msg.fsw_state.state)
        {
        case SM_STATE_INIT:
        ...
        {
            panel.rbf_neo = lora_recv_data.msg.fsw_state.state == SM_STATE_WAIT_GS ? BLUE : RED;
        }
        else
        {
            panel.rbf_neo = GREEN;
        ...
        if (lora_recv_data.msg.fsw_state.state <= SM_STATE_READY && panel.rbf_btn && panel.rbf_key_btn)
        {
        ...
        if (lora_recv_data.msg.fsw_state.state == SM_STATE_WAIT_GS && panel.rbf_btn && panel.rbf_key_btn)
        {
        ...
        */
        ...
        if (dev->state == RX && (dio2 || dio3))
        {
            ...
            else if (dio2) // RxDone
            {
                ...
                if (msg_fsw.fsw_state.state <= SM_STATE_FAIL)
                {
                    pthread_mutex_lock(&lora_recv_data.lock);
                    pthread_mutex_lock(&lora_recv_data_api.lock);
                    memcpy(&lora_recv_data.msg, &msg_fsw, sizeof(msg_fsw));

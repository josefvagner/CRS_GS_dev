...
pio_sync_setup(struct can2040 *cd)
{
    ...
    sm->execctrl = (cd->gpio_rx << PIO_SM0_EXECCTRL_JMP_PIN_LSB | (can2040_offset_sync_end - 1) << PIO_SM0_EXECCTRL_WRAP_TOP_LSB | can2040_offset_sync_signal_start << PIO_SM0_EXECCTRL_WRAP_BOTTOM_LSB);
    sm->pinctrl = (1 << PIO_SM0_PINCTRL_SET_COUNT_LSB | cd->gpio_rx << PIO_SM0_PINCTRL_SET_BASE_LSB);
...
pio_rx_setup(struct can2040 *cd)
{
    ...
    sm->execctrl = ((can2040_offset_shared_rx_end - 1) << PIO_SM0_EXECCTRL_WRAP_TOP_LSB | can2040_offset_shared_rx_read << PIO_SM0_EXECCTRL_WRAP_BOTTOM_LSB);
    sm->pinctrl = cd->gpio_rx << PIO_SM0_PINCTRL_IN_BASE_LSB;
    sm->shiftctrl = 0; // flush fifo on a restart
    sm->shiftctrl = (PIO_SM0_SHIFTCTRL_FJOIN_RX_BITS | PIO_RX_WAKE_BITS << PIO_SM0_SHIFTCTRL_PUSH_THRESH_LSB | PIO_SM0_SHIFTCTRL_AUTOPUSH_BITS);
...
pio_tx_setup(struct can2040 *cd)
{
    ...
    sm->execctrl = (cd->gpio_rx << PIO_SM0_EXECCTRL_JMP_PIN_LSB | can2040_offset_tx_conflict << PIO_SM0_EXECCTRL_WRAP_TOP_LSB | can2040_offset_tx_conflict << PIO_SM0_EXECCTRL_WRAP_BOTTOM_LSB);
    sm->shiftctrl = (PIO_SM0_SHIFTCTRL_FJOIN_TX_BITS | PIO_SM0_SHIFTCTRL_AUTOPULL_BITS);
    sm->pinctrl = (1 << PIO_SM0_PINCTRL_SET_COUNT_LSB | 1 << PIO_SM0_PINCTRL_OUT_COUNT_LSB | cd->gpio_tx << PIO_SM0_PINCTRL_SET_BASE_LSB | cd->gpio_tx << PIO_SM0_PINCTRL_OUT_BASE_LSB);
...
pio_tx_reset(struct can2040 *cd)
{
    ...
    pio_hw->ctrl = ((0x07 << PIO_CTRL_SM_ENABLE_LSB) | (0x08 << PIO_CTRL_SM_RESTART_LSB));
...
pio_tx_did_fail(struct can2040 *cd)
{
    ...
    return (!(pio_hw->flevel & PIO_FLEVEL_TX3_BITS) && (pio_hw->intr & (SI_MAYTX | SI_RX_DATA)) == SI_MAYTX);
}
...
tx_check_local_message(struct can2040 *cd)
{
    ...
    if (tm->id == pm->id)
    {
        if (qt->crc != cd->parse_crc || tm->dlc != pm->dlc || tm->data32[0] != pm->data32[0] || tm->data32[1] != pm->data32[1])
...
{
    if (cd->report_state & RS_NEED_EOF_FLAG)
    { // RS_NEED_xX_EOF
...
report_line_txpending(struct can2040 *cd)
{
    ...
    if (pio_irqs == (SI_MAYTX | SI_TXPENDING | SI_RX_DATA) && cd->report_state == RS_NEED_RX_ACK)
    {
...
data_state_update_ext_header(struct can2040 *cd, uint32_t data)
{
    ...
    uint32_t id = (((hdr1 << 11) & 0x1ffc0000) | ((hdr1 << 13) & 0x3e000) | (data >> 7) | CAN2040_ID_EFF);

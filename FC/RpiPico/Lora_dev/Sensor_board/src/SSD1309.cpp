...
*/
SSD1309::SSD1309(uint8_t const width, uint8_t const height, spi_inst_t *spi, uint8_t resetPin, uint8_t csPin, uint8_t dcPin) : width(width), height(height), spi(spi), resetPin(resetPin), csPin(csPin), dcPin(dcPin)
{
	gpio_init(this->resetPin);
	gpio_set_dir(this->resetPin, GPIO_OUT);
	gpio_pull_up(this->resetPin);

	gpio_init(this->csPin);
	gpio_set_dir(this->csPin, GPIO_OUT);
	gpio_put(this->csPin, 1);

	gpio_init(this->dcPin);
	gpio_set_dir(this->dcPin, GPIO_OUT);
	gpio_put(this->dcPin, 1);
...
void SSD1309::sendCommand(uint8_t command)
{
	...
	// printf("sending to spi : %d\n", this->spi);
	gpio_put(this->dcPin, 0);
	gpio_put(this->csPin, 0);
...
void SSD1309::sendData(uint8_t *buffer, size_t buff_size)
{
	...
	gpio_put(this->dcPin, 1);
	gpio_put(this->csPin, 0);
	spi_write_blocking(this->spi, buffer, buff_size);
	gpio_put(this->csPin, 1);
}

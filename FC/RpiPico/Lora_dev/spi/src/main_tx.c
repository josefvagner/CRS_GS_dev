...
int main()
{
    ...
    SetDioIrqParams(&dev, 0b0100000000000001, 0b0000000000000001, 0b0100000000000000, 0);
    SetTxParams(&dev, 0x1F, 0x00);
    ...
    while (true)
    {
        ...
        WriteBuffer(&dev, buff, sizeof(buff));
        SetTx(&dev, 0x02, 0);
        while (true)
        {
            irq = GetIrqStatus(&dev);
            if (irq & 0x01)
            {
                tnow = time_us_64();
